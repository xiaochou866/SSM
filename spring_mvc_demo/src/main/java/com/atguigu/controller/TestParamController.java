...
* 2. 通过控制器方法的形参获取
* 只需要在控制器方法的形参位置，设置一个形参，形参的名字和请求的参数的名字一致
* 3.@RequestParam: 将请求参数和控制器方法的形参绑定
* @RequestParam注解的三个属性: value, required, defaultValue
* value: 设置和形参绑定的请求参数的名字
* required: 设置是否必须传输value所对应的请求参数
* 默认值为true 标识value所对应的请求参数必须传输，否则页面报错
* 若设置为false,则标识value所对应的请求参数不是必须传输，若为传输，则形参值为Null
* defaultValue: 设置当没有传输value所对应的请求参数时，为形参设置默认值，此时和required的属性值一致
* 4.@RequestHeader: 将请求头信息和控制器方法为的形参绑定
...
public class TestParamController {
    ...
    // 在一些特定的情况下会使用RequestParam指定请求的变量名
