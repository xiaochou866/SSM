...
public class IOCByAnnotationTest {
    /*
    * @Conmponent 将类标识为普通组件
      @Controller 将类标识为控制层组件
      @Service 将类标识为业务层组件
      @Repository 将类标识为持久层组件
      *
      * 通过注解+扫描所配置的bean的id, 默认值为类的小驼峰， 即类名的小字母为小写的结果
      * 可以通过标识组件的注解的value属性值设置bean的自定义的id
      *
      * @Atuowired: 实现自动装配功能的注解
      ...
      *  b>若有多个类型匹配的Bean此时会自动转化为byName的方式来实现自动装配的效果
      *     即将要赋值的属性的属性名作为bean的id匹配某个bean为属性赋值
      *  c>byType的话对应类型的bean有多个 byName的话没有一个bean与属性名对应的bean存在
